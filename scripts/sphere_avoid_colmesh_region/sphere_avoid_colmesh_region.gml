/// @description smf_sphere_avoid_colmesh_region(colmesh, region, x, y, z, radius, xup, yup, zup, slope(degrees))
/// @param colmesh
/// @param region
/// @param x
/// @param y
...
var success, newX, newY, newZ, xup, yup, zup, R, R2, slope;
success = false;
newX = argument2;
newY = argument3;
newZ = argument4;
...
yup = argument7;
zup = argument8;
slope = dcos(argument9);

var rnX, rnY, rnZ, maxDP;
maxDP = -1;
rnX = 0;
rnY = 0;
rnZ = 0;

var transformed = is_array(colMesh[| eColMesh.Matrix])
if transformed
{
...
if region > 0
{
	var priority, i, j, tri, D, j1, j2, t0, t1, t2, u0, u1, u2, dp, d0, d1, d2, l, _r, _l;
...
if transformed
{
	...
	_nX = (M[0] * rnX + M[4] * rnY + M[8]  * rnZ) * scale;
	_nY = (M[1] * rnX + M[5] * rnY + M[9]  * rnZ) * scale;
	_nZ = (M[2] * rnX + M[6] * rnY + M[10] * rnZ) * scale;
	rnX = _nX;
	rnY = _nY;
	rnZ = _nZ;
}
